  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8 pb-12">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-white">Wallet & Points</h1>
        <div className="flex items-center gap-2 rounded-full bg-neutral-900 px-4 py-2 text-sm font-bold text-neutral-500 border border-white/5">
          <Clock size={18} />
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        {/* Balance Card */}
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-orange-600 to-orange-500 p-8 text-white shadow-2xl shadow-orange-900/20">
        ...
        <div className="rounded-3xl border border-white/5 bg-neutral-900 p-8 shadow-xl ring-1 ring-white/5">
          <div className="mb-8 flex items-center justify-between">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-orange-600/10 text-orange-500">
              <Award size={24} />
            </div>
          ...
          <p className="text-sm font-bold uppercase tracking-widest text-neutral-500">Total Points</p>
          <h2 className="text-5xl font-bold tracking-tight text-white">{user.points || 0}</h2>
          <div className="mt-8">
            <button 
              onClick={handleConvertPoints}
              disabled={isProcessing || (user.points || 0) < 100}
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-orange-600 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-orange-900/20 hover:bg-orange-700 transition-all active:scale-95 disabled:opacity-50"
            >
              {isProcessing ? <Loader2 className="animate-spin" size={18} /> : <DollarSign size={18} />}
              Convert to Cash
            </button>
            <p className="mt-2 text-center text-[10px] font-bold text-neutral-500 uppercase tracking-widest">
              100 points = $1.00
            </p>
          </div>
        </div>
      ...
      {showDeposit && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-3xl bg-neutral-900 p-8 shadow-2xl border border-white/5">
            <h3 className="mb-4 text-xl font-bold text-white">Manual Deposit</h3>
            ...
            <div className="mb-6">
              <label className="mb-2 block text-xs font-bold uppercase tracking-widest text-neutral-500">Amount ($)</label>
              <input
                type="number"
                value={depositAmount}
                onChange={(e) => setDepositAmount(e.target.value)}
                placeholder="0.00"
                className="w-full rounded-xl border border-white/5 bg-neutral-950 p-4 text-2xl font-bold text-white focus:border-orange-600 focus:outline-none"
              />
            </div>
            <div className="flex gap-4">
              <button 
                ...
                className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-orange-600 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-orange-900/20 hover:bg-orange-700 transition-all disabled:opacity-50"
              >
                {isProcessing ? <Loader2 className="animate-spin" size={18} /> : 'Submit Request'}
              </button>
              <button 
                onClick={() => setShowDeposit(false)}
                className="flex-1 rounded-xl bg-neutral-800 px-4 py-3 text-sm font-bold text-neutral-300 hover:bg-neutral-700 transition-all"
              >
                Cancel
              </button>
            </div>
      ...
      <div className="rounded-3xl border border-white/5 bg-neutral-900 shadow-xl ring-1 ring-white/5">
        ...
        <div className="p-6">
          {loading ? (
            <div className="py-8 text-center text-neutral-500">Loading transactions...</div>
          ) : transactions.length === 0 ? (
            <div className="py-8 text-center text-neutral-500">No transactions yet.</div>
          ) : (
            <div className="space-y-4">
              {transactions.map((tx) => (
                <div key={tx.id} className="flex items-center justify-between rounded-2xl bg-neutral-950 p-4 border border-white/5">
                  ...
                  <div className="text-right">
                    <p className={`text-sm font-bold ${
                      tx.type === 'deposit' || tx.type === 'points_conversion' ? 'text-green-500' : 'text-red-500'
                    }`}>
